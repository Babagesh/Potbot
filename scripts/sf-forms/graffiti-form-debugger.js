...
/**
 * Graffiti Form Structure Debugger - Investigate the graffiti form structure
 */
class GraffitiFormDebugger {
  constructor(options = {}) {
  ...
  /**
   * Debug graffiti form structure
   */
  async debugGraffitiForm() {
    console.log('🎨 Debugging Graffiti Form Structure');
    console.log('====================================');
    ...
    try {
      ...
      console.log('🌐 Navigating to graffiti form...');
      ...
      console.log('🔍 Analyzing graffiti form structure...');
  ...
  async analyzeFormStructure() {
    ...
    // Look for all dropdowns/selects
    ...
    dropdowns.forEach((dropdown, i) => {
      console.log(`     ${i + 1}. Name: "${dropdown.name}", ID: "${dropdown.id}"`);
      console.log(`        Class: ${dropdown.className}`);
      console.log(`        Visible: ${dropdown.isVisible}, Enabled: ${dropdown.isEnabled}`);
      if (dropdown.isVisible && dropdown.options.length > 1) {
        console.log(`        Options:`);
    ...
    const allButtons = await this.page.evaluate(() => {
      ...
      return buttons
        ...
        .filter(btn => btn.text && btn.isVisible); // Only show buttons with text and visible
    });
    
    console.log(`   Found ${allButtons.length} visible buttons with text:`);
...
// Export the class
module.exports = { GraffitiFormDebugger };

// Run test if this file is executed directly
if (require.main === module) {
  async function runGraffitiDebug() {
    const debuggerInstance = new GraffitiFormDebugger({ headless: false });
    
    try {
      const result = await debuggerInstance.debugGraffitiForm();
      ...
      console.error('Graffiti form debug failed:', error);
    }
  }

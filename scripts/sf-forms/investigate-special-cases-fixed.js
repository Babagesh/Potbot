    ...
    try {
      ...
      if (reportButton) {
        ...
        const pageInfo = await page.evaluate(() => {
          ...
          const hasEmergencyText = /emergency|911|call.*police|call.*fire/i.test(document.body.textContent);
          
          // Check for buttons using standard DOM methods
          const nextButton = Array.from(document.querySelectorAll('button, a, input[type="submit"], input[type="button"]'))
            .find(el => /next/i.test(el.textContent || el.value || ''));
          
          const continueButton = Array.from(document.querySelectorAll('button, a, input[type="submit"], input[type="button"]'))
            .find(el => /continue/i.test(el.textContent || el.value || ''));
          
          const submitButton = Array.from(document.querySelectorAll('button, a, input[type="submit"], input[type="button"]'))
          ...
          return {
            ...
            hasEmergencyText: hasEmergencyText,
            hasNextButton: nextButton !== undefined,
            hasContinueButton: continueButton !== undefined,
            hasSubmitButton: submitButton !== undefined,
        ...
        if (pageInfo.hasForm && !pageInfo.hasEmergencyText) {
          ...
          // Check if this form is ready for filling
          if (formFields.length > 0) {
            console.log('✅ This form appears to be ready for data entry');
            console.log('💡 No emergency disclaimer step needed for this form type');
          }
        }
        
        // Check for any navigation elements
        console.log('🔍 Step 6: Looking for navigation elements...');
        const navElements = await page.evaluate(() => {
          const elements = Array.from(document.querySelectorAll('a, button, input[type="submit"], input[type="button"]'))
            ...
            .filter(el => el.text && el.isVisible && /next|continue|proceed|submit|start|begin/i.test(el.text));
